...
#ChangeDicts = []
#ChangeDicts.append({'WORD': r'P1: pitchPGain=', 'EXCHANGE': r'P1: pitchPGain='})  # .85'})
#ChangeDicts.append({'WORD': r'P2: pitchIGain=', 'EXCHANGE': r'P2: pitchIGain='})  # .85'})

ChangeValues1 = []
Initial_Value = 0.43404
for i in range(15):
    ChangeValues1.append(Initial_Value)
    Initial_Value = Initial_Value * 0.9
ChangeValues1 = [i/100 for i in range(0, 41, 4)]
print(ChangeValues1)
ChangeValues2 = []
Initial_Value = 0.03006
for i in range(15):
    ChangeValues2.append(Initial_Value)
    Initial_Value = Initial_Value * 0.9
ChangeValues2 = [1/i for i in range(10, 100, 5)]
print(ChangeValues2)
...
folder = r'H:\BladedWS\FOWTs\CheckPI_gains\\'
folder = r'H:\BladedWS\FOWTs\iterTowerDamp2\\'
...
for idx, fileName in enumerate(ListOfBaselineFiles):
    for value1 in ChangeValues1:
        for value2 in ChangeValues2:
            NameAdd = '_itr_FA_G' + str('%.02f' % value1).replace('.', '_') + '_Hz' + str('%.03f' % value2).replace('.', '_')
            #NameAdd = '_gain_P' + str('%.04f' % value1).replace('.', '_') + '_I' + str('%.04f' % value2).replace('.', '_')
                ...
                for idx2, Dict in enumerate(ChangeDicts):
                    if row.find(Dict.get('WORD')) != -1:
                        ...
                        if idx2 == 0:
                            ...
                            value = value2
                        row = Dict.get('EXCHANGE') + str('%.04f' % value) + ';' + '\n'
                        print('is now: ' + row)
                outfile.write(row)

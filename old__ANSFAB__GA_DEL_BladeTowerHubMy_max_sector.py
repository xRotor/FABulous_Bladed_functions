...
import copy
import os
from ANSFAB___Customize_GA import time_total, deltat, nSectors, eps, Referenz_Frequency, k, var_nbins, PrintDetails,\
    fileEnd, nBlades
...
# deltat = 0.01       # time step length in s
# nSectors = 12 # 24
...
def calcDELfromTimeSeries(timeSeries):
    DEL = 0.0
...
def calcWorstDELsector(Mx_timeSeries, My_timeSeries, nSectors, MainResults, file):
    ...
    for n in range(nSectors):
        ...
        DEL_Sector[n] = calcDELfromTimeSeries(My_timeSeries_Sector[n][:])
    ...
    DELx = calcDELfromTimeSeries(Mx_timeSeries)
    DELy = calcDELfromTimeSeries(My_timeSeries)
    DEL_ref = calcDELfromTimeSeries(Mxy_timeSeries)
...
def extractDEL_towerHubBlade(RunFolder, ListOfBladedJobs):
    ...
    for idx, filename in enumerate(ListOfBladedJobs): #[11:500]:
        #break
        file = os.path.join(RunFolder, filename).replace(fileEnd, towerFileEnd)
        if PrintDetails:
        ...
        DEL_max = calcWorstDELsector(TowerMx, TowerMy, nSectors, MainResults, file)
    ...
    for idx, filename in enumerate(ListOfBladedJobs):  # [11:500]:
        file = os.path.join(RunFolder, filename).replace(fileEnd, hubFileEnd)
        if PrintDetails:
            print('current file is called: ', file)  # filename = (file.replace(RunFolder + '\\', '', ))
...
#        DEL_max, MainResults = calcWorstDELsector(HubMz, HubMy, nSectors, MainResults, file)
#        DEL_dicts[idx]['Hub_My_sector_max_DEL'] = DEL_max
        DEL_dicts[idx]['Hub_Mx_DEL'] = calcDELfromTimeSeries(HubMx)
    ...
    for idx, filename in enumerate(ListOfBladedJobs):  # [11:500]:
        # bladeFileEnd = '.$41'
        pitchFileEnd = '.$08'
        Pitch_movement_file = os.path.join(RunFolder, filename).replace(fileEnd, pitchFileEnd)

        Sum_Blade_My_DELs = 0
        Sum_Pitch_LDCs = 0
        for idx_blade in range(nBlades):
            bladeFileEnd = '.$4' + str(idx_blade + 1)
            Blade_load_file = os.path.join(RunFolder, filename).replace(fileEnd, bladeFileEnd)
            ...
            Sum_Blade_My_DELs += calcDELfromTimeSeries(BladeMy)
